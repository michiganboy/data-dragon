...
function generateCSV(summary, userMap, allWarnings, userActivities) {
  try {
    ...
    userActivities.forEach((activity, userId) => {
      ...
      csvRows = csvRows.concat(activityRows);
    });
...
function formatWarningMessage(warning) {
  ...
  // Look for geographic location change messages that need reformatting
  if (warning.includes('CRITICAL - Geographic location change:') || 
      warning.includes('CRITICAL - Suspicious login location change:')) {
    
    // Clean up any poorly formatted location change messages
    return warning.replace(/CRITICAL - .*?(Geographic|Suspicious) location change: /, 'Suspicious login location change: ')
  ...
  // Handle content sharing warnings differently 
  if (warning.includes('High content sharing rate') || 
      warning.includes('Excessive Internal Sharing') ||
      warning.includes('Content sharing:')) {
    
    // Format content sharing messages
    let formatted = warning;
    
    // Make sure it has a content sharing prefix
    if (!formatted.startsWith('Content sharing:') && 
        !formatted.startsWith('Warning - Content sharing:')) {
      formatted = 'Content sharing: ' + formatted;
    }
  ...
  // General cleanup for any warning message
  return warning.replace(/ \(\d+ points\)/, '')
                .replace(/Warning - /, '');
}
