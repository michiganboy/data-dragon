...
async function generatePDFReport(config, userActivities, options = {}) {
  try {
    ...
    if (!pdfReportGenerator) {
      throw new Error('PDF reporting dependencies not available. Install pdfkit, chart.js, and chartjs-node-canvas packages.');
    }
    
    // Default output path based on config
    const outputPath = options.outputPath || 
                       path.join(path.dirname(config.SUMMARY_CSV), 'security-report.pdf');
