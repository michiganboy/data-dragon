...
let pdfReportGenerator;
try {
  pdfReportGenerator = require('./reporting/pdf/generator');
} catch (error) {
  // PDF reporting dependencies might not be installed
  pdfReportGenerator = null;
}
...
async function generatePDFReport(config, userActivities, options = {}) {
  try {
    ...
    if (!pdfReportGenerator) {
      throw new Error('PDF reporting dependencies not available. Install pdfkit package.');
    }
    ...
    const pdfPath = await pdfReportGenerator.generateSecurityReport({
      userActivities,
      outputPath,
      reportOptions
    });
...
function isPDFReportingAvailable() {
  return pdfReportGenerator !== null && pdfReportGenerator.isPdfGenerationAvailable();
}
