...
/**
 * Generate a banner with summary statistics for the console
...
function generateSummaryBanner(summary) {
  try {
    ...
    const bannerContent = [
      ...
      ` - PDF: ${
        stats.pdfGenerated ? config.PDF_REPORT_PATH : "Not generated"
      }`,
    ];
...
/**
 * Generate a PDF security report using HTML and Puppeteer
 * @param {Object} config - Application configuration
 * @param {Map<string, UserActivity>} userActivities - Map of userId -> UserActivity
 * @param {Object} options - PDF report options
 * @returns {Promise<string|null>} Path to the generated PDF or null if failed
 */
async function generatePDFReport(config, userActivities, options = {}) {
  try {
    // Check if HTML reporting is available
    if (!htmlReportGenerator) {
      throw new Error('HTML reporting dependencies not available. Install ejs and puppeteer packages.');
    }
    
    // Validate userActivities
    if (!userActivities || typeof userActivities.forEach !== 'function') {
      utils.log("warn", "No valid user activities data available for PDF report.");
      ...
      if (!(userActivities instanceof Map)) {
        utils.log("info", "Creating minimal user activities data for PDF report");
    ...
    const outputPath = options.outputPath || 
                       path.join(path.dirname(config.SUMMARY_CSV), 'reports', 'security-report.pdf');
    ...
    const pdfPath = await htmlReportGenerator.generateSecurityReport({
      userActivities,
      outputPath,
      reportOptions
    });
    
    utils.log("info", `Generated PDF security report at ${pdfPath}`);
    return pdfPath;
  } catch (error) {
 ...
 */
function isPDFReportingAvailable() {
  return htmlReportGenerator !== null && htmlReportGenerator.isPdfGenerationAvailable();
}
