...
function checkForRisks(row, eventType) {
  ...
  // Debug for ApexExecution
  if (eventType === "ApexExecution") {
    console.log(`DEBUG checkForRisks: Processing ApexExecution event`);
  }
  ...
  // Debug for ApexExecution
  if (eventType === "ApexExecution") {
    console.log(`DEBUG checkForRisks: UserKey=${userKey}, HourKey=${hourKey}`);
  }
  ...
  // Debug for ApexExecution
  if (eventType === "ApexExecution") {
    console.log(`DEBUG checkForRisks: TimeWindow=${config.timeWindow || 'day'}`);
  }
  ...
  if (config.countField) {
    ...
    // Debug for ApexExecution
    if (eventType === "ApexExecution") {
      console.log(`DEBUG checkForRisks: Using countField=${config.countField}, Value=${fieldValue}, CurrentCount=${counterGroup[counterKey]}, Threshold=${config.threshold}`);
      console.log(`DEBUG checkForRisks: AlertedEvents=${JSON.stringify(counterGroup.alertedEvents)}`);
    }
    ...
    if (counterGroup[counterKey] >= config.threshold && !counterGroup.alertedEvents[alertKey]) {
      // Debug for ApexExecution
      if (eventType === "ApexExecution") {
        console.log(`DEBUG checkForRisks: Threshold met, generating alert`);
      }
    ...
    } else if (eventType === "ApexExecution") {
      // Debug for ApexExecution
      console.log(`DEBUG checkForRisks: Threshold check failed or already alerted`);
    }
  } else {
    ...
    // Debug for ApexExecution
    if (eventType === "ApexExecution") {
      console.log(`DEBUG checkForRisks: Using eventType as key, CurrentCount=${counterGroup[counterKey]}, Threshold=${config.threshold}`);
      console.log(`DEBUG checkForRisks: AlertedEvents=${JSON.stringify(counterGroup.alertedEvents)}`);
    }
  ...
  if (config.customDetection && typeof config.customDetection === "function") {
    // Debug for ApexExecution
    if (eventType === "ApexExecution") {
      console.log(`DEBUG checkForRisks: Running custom detection`);
    }
    ...
    try {
      ...
      if (customResult) {
        // Debug for ApexExecution
        if (eventType === "ApexExecution") {
          console.log(`DEBUG checkForRisks: Custom detection generated alert`);
          console.log(`DEBUG checkForRisks: CustomMessage=${customResult.customMessage}`);
        }
        ...
        // Debug for ApexExecution
        console.log(`DEBUG checkForRisks: Custom detection returned null (no alert)`);
      }
    } catch (error) {
      utils.log(
        "error",
      ...
      // Debug for ApexExecution
      if (eventType === "ApexExecution") {
        console.log(`DEBUG checkForRisks: Error in custom detection - ${error.message}`);
      }
    }
  }
}
...
function logRisk(userKey, row, warning, severity = "medium") {
  ...
  if (!exists) {
    ...
    utils.log(
      "info",
      severityColor(
        `${severityText} ${severity.toUpperCase()}: ${warning} by ${username}`
      )
    );

    // Conditional detailed logging for higher severity issues
    if (severity === "critical" || severity === "high") {
      const contextStr = Object.entries(warningObj.context)
        .map(([k, v]) => `${k}: ${v}`)
        .join(" | ");

      if (contextStr) {
        utils.log("debug", chalk.gray(`   Context: ${contextStr}`));
      }
    }
  }
}
