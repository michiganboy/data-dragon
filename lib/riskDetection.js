...
async function startProcessing(tokens, options = {}) {
  ...
  try {
    ...
    const reporting = require("./reporting");
    const summary = reporting.generateSummary(config, userActivities, userMap);
    ...
    if (options.pdf) {
      ...
      if (reporting.isPDFReportingAvailable()) {
        ...
        const pdfOptions = {
          ...options.pdfOptions, // Existing options from command line
    ...
    try {
      ...
      const reporting = require("./reporting");
      reporting.generateSummary(config, userActivitiesMap, userMap);
      
      // Try to generate PDF report if requested, even with minimal data
      if (options.pdf && reporting.isPDFReportingAvailable()) {
...
function logRisk(userKey, row, warning, severity = "medium") {
  ...
  // For location-based warnings, add a type prefix to help with formatting later
  if (warning.includes('location change:')) {
    console.log(`[DEBUG] Handling location change warning: ${warning}`);
    warningObj.warning = `LOCATION_CHANGE: ${warning}`;
    console.log(`[DEBUG] Modified to: ${warningObj.warning}`);
  }
  ...
  if (!exists) {
    ...
    // Log the warning with proper severity labeling
    utils.log(
      "risk",
      `${severityLabel} ${warning} by ${username}`
    );

    console.log(`[DEBUG] Adding warning: ${JSON.stringify(warningObj)}`);
    allWarnings.push(warningObj);
  } else {
    console.log(`[DEBUG] Warning already exists, skipping: ${warning}`);
  }
}
