...
async function startProcessing(tokens, options = {}) {
  ...
  try {
    ...
    console.log("🐲 DEBUG: About to add warnings. allWarnings count:", allWarnings.length);
    console.log("🐲 DEBUG: userActivities type:", typeof userActivities);
    console.log("🐲 DEBUG: userActivities size:", userActivities ? userActivities.size : "null");
    
    loginDataManager.addWarnings(allWarnings);

    // Step 7: Perform risk correlation analysis
    console.log("🐲 DEBUG: Step 7 - Starting risk correlation analysis...");
    utils.log(
      "info",
    ...
    console.log("🐲 DEBUG: Risk correlation analysis completed");
    ...
    console.log("🐲 DEBUG: Step 8 - About to generate summary...");
    console.log("🐲 DEBUG: userMap keys:", Object.keys(userMap));
    console.log("🐲 DEBUG: allWarnings length:", allWarnings.length);
    console.log("🐲 DEBUG: userActivities size:", userActivities ? userActivities.size : "null");
    ...
    let summaryData;
    try {
      summaryData = reporting.generateSummary(
        userMap,
        allScannedFiles,
        allWarnings,
        userActivities,
      ...
      console.log("🐲 DEBUG: Summary generation successful");
    } catch (summaryError) {
      console.log("🐲 DEBUG: Summary generation failed:", summaryError.message);
      console.log("🐲 DEBUG: Summary error stack:", summaryError.stack);
      throw summaryError;
    }
    ...
    console.log("🐲 DEBUG: Step 9 - Starting HTML report generation...");
    try {
      if (reporting.isReportingAvailable()) {
        ...
        console.log("🐲 DEBUG: Calling generateReport with options:", reportOptions);
        // Generate the HTML report
        await reporting.generateReport(config, userActivities, reportOptions);
        console.log("🐲 DEBUG: HTML report generation completed successfully");
      } else {
        console.log("🐲 DEBUG: HTML reporting not available");
        utils.log(
          "warn",
          "HTML report generation not available. Install required packages (ejs)."
        );
      }
    } catch (reportError) {
      console.log("🐲 DEBUG: HTML report generation failed:", reportError.message);
      console.log("🐲 DEBUG: HTML report error stack:", reportError.stack);
      utils.log("error", `Error generating HTML report: ${reportError.message}`);
      
      // Still show completion message even if HTML report failed
      utils.log("info", "Scan completed successfully. JSON and CSV reports generated.");
    }
...
/**
 * Generate HTML report with appropriate error handling
 * @param {Object} config - Application configuration
 * @param {Map<string, Object>} userActivities - User activities map
 */
async function generateHTMLReport(config, userActivities) {
  try {
    const reporting = require("./reporting");
    if (!reporting.isReportingAvailable()) {
      utils.log(
    ...
    const reportOptions = {
      title: 'DataDragon Security Analysis Report',
      organization: config.ORGANIZATION_NAME || 'Your Organization',
      outputPath: path.join(process.cwd(), "output/reports", "security-report.html")
    };
    
    // Generate the HTML report
    const htmlPath = await reporting.generateReport(config, userActivities, reportOptions);
    
    if (htmlPath) {
      utils.log("info", chalk.green(`HTML report generated successfully at: ${htmlPath}`));
    }
    
    return htmlPath;
  } catch (error) {
    utils.log("error", `Error generating HTML report: ${error.message}`);
    return null;
  }
}
