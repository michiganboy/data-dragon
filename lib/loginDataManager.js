...
class LoginDataManager {
  ...
  addWarnings(warnings) {
    ...
    // Then ensure all geographic anomalies are also added as warnings
    this.userActivities.forEach((activity, userId) => {
      if (activity.anomalies && activity.anomalies.length > 0) {
        // Look for rapid IP change anomalies
        const locationAnomalies = activity.anomalies.filter(
          anomaly => anomaly.type === 'rapid_ip_change' && anomaly.details && anomaly.details.geoInfo
        );
        
        console.log(`[DEBUG] Found ${locationAnomalies.length} location anomalies for user ${activity.username}`);
        
        // Create warnings for these anomalies
        locationAnomalies.forEach(anomaly => {
          // Format time difference for better readability
          const hoursValue = parseFloat(anomaly.details.hours || "0");
          let timeDiff = `${hoursValue} hours`;
          if (hoursValue < 0.016) { // Less than 1 minute
            timeDiff = `${Math.round(hoursValue * 60 * 60)} seconds`;
          } else if (hoursValue < 1) { // Less than 1 hour
            timeDiff = `${Math.round(hoursValue * 60)} minutes`;
          }
          
          // Format the warning message
          const warningMessage = `Suspicious login location change: ${anomaly.details.prevLocation} to ${anomaly.details.currLocation} (${timeDiff} apart)`;
          
          console.log(`[DEBUG] Creating explicit warning for location change: ${warningMessage}`);
          
          // Create a warning object
          const warning = {
            userId: userId,
            user: activity.username,
            date: anomaly.details.prevTime ? anomaly.details.prevTime.split('T')[0] : new Date().toISOString().split('T')[0],
            timestamp: anomaly.details.prevTime || new Date().toISOString(),
            warning: warningMessage,
            severity: "critical",
            eventType: "LocationChange", // Use a more distinctive event type
            priority: true, // Mark this as a priority warning for reporting
            clientIp: anomaly.details.from || "",
            sessionKey: "",
            context: {
              to_ip: anomaly.details.to || "",
              prev_location: anomaly.details.prevLocation || "",
              curr_location: anomaly.details.currLocation || "",
              hours_apart: anomaly.details.hours || ""
            }
          };
          
          // Add the warning to the user activity
