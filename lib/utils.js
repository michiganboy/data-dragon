...
function log(level, message) {
  ...
  // Never show debug logs unless explicitly set in environment
  const shouldDisplay = 
    level !== 'debug' && (
      logLevels[level]?.display || 
      level === envLogLevel || 
      (envLogLevel === "debug" && level !== 'debug')
    );
  ...
  if (shouldDisplay) {
    ...
    if (level === 'risk') {
      // Apply colors based on message severity
      if (message.includes('[CRITICAL]')) {
        colorFn = chalk.red.bold;
      } else if (message.includes('[HIGH]')) {
        colorFn = chalk.hex('#FF8C00').bold; // Dark orange for HIGH
      } else if (message.includes('[MEDIUM]')) {
        colorFn = chalk.yellow; // Yellow for MEDIUM
      } else if (message.includes('[LOW]')) {
        colorFn = chalk.blue; // Blue for LOW
      }
