...
function createSecurityReport(data, options = {}) {
  ...
  // Title page
  createTitlePage(doc, data, options);
  
  // Table of contents
  createTableOfContents(doc, data);
  
  // Executive summary
  createExecutiveSummary(doc, data);
  
  // Risk analysis
  createRiskAnalysis(doc, data);
  
  // User activity details
  createUserDetails(doc, data);
  
  // Appendix with detailed warnings
  if (options.includeAppendix !== false) {
    createAppendix(doc, data, options);
  }
