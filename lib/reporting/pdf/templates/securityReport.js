...
const chartGenerator = require('../components/chartGenerator');
const { 
  wrapText, 
  formatDate, 
  loadImage, 
...
function createSecurityReport(data, options = {}) {
  ...
  const doc = new PDFDocument({
    ...
    margin: styleConfig.page.margins,
    info: {
      Title: options.title || 'Security Analysis Report',
      Author: 'DataDragon Security Monitor',
      Subject: 'Salesforce Security Analysis',
      Keywords: 'security, salesforce, monitoring, analysis',
  ...
  addExecutiveSummary(doc, data, options);
  
  // Add risk distribution section with charts
  addRiskDistributionSection(doc, data, options);
  
  // Add high risk users section
  addHighRiskUsersSection(doc, data, options);
  
  // Add warnings by event type section
  addEventTypeSection(doc, data, options);
  
  // Add detailed findings sections
  if (!options.skipDetailedFindings) {
    addDetailedFindingsSection(doc, data, options);
  }
  
  // Add appendix if enabled
  if (options.includeAppendix) {
    addAppendixSection(doc, data, options);
  }
...
async function addExecutiveSummary(doc, data, options) {
  ...
  doc.fontSize(styleConfig.fonts.header.size.h2)
     ...
     .font('Helvetica')
     .text(' ' + formatDate(options.scanDate || new Date()));
...
async function addRiskDistributionSection(doc, data, options) {
  ...
  try {
    // Generate charts - first prepare data
    const severityData = {
    ...
    // Generate pie chart for severity distribution
    const pieChartBuffer = await chartGenerator.generatePieChart(severityData, {
...
async function addEventTypeSection(doc, data, options) {
  ...
  try {
    // Generate chart data
    const chartData = {
      labels: eventTypeArray.slice(0, 8).map(type => type.name),
      datasets: [{
        label: 'Event Count',
        data: eventTypeArray.slice(0, 8).map(type => type.count),
        backgroundColor: [
    ...
    const chartBuffer = await chartGenerator.generateBarChart(chartData, {
