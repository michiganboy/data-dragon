...
async function startProcessing(tokens, options = {}) {
  ...
  try {
    ...
    // Step 4: Fetch available event logs
    const query = `SELECT Id,LogFile,EventType,LogDate FROM EventLogFile ORDER BY LogDate DESC`;
...
function checkForRisks(row, eventType) {
  ...
  const userKey = `${row.USER_ID}-${row.EVENT_DATE}`;
  const sessionKey = row.SESSION_KEY || "unknown-session";
  const hourKey = `${row.EVENT_DATE}-${new Date(
  ...
  if (config.countField) {
    ...
    // Check if threshold is reached for this specific value
    if (counterGroup[counterKey] >= config.threshold) {
...
function logRisk(userKey, row, warning, severity = "medium") {
  ...
  const timestamp =
    row.EVENT_TIME || row.TIMESTAMP || row.LOGIN_TIME || row.EVENT_DATE;
  ...
  const warningObj = {
    ...
    clientIp: row.CLIENT_IP || row.SOURCE_IP || "Unknown",
